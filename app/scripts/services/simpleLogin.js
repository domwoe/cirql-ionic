...
angular.module('simpleLogin', ['firebase', 'firebase.utils', 'cirqlApp.config'])
...
.factory('createProfile', function(fbutil, $q, $timeout) {
  return function(id, email, name) {
    var ref = fbutil.ref('homes', id), def = $q.defer();
...
.factory('changeEmail', function(fbutil, $q) {
  return function(password, oldEmail, newEmail, simpleLogin) {
    ...
    function loadOldProfile() {
      var def = $q.defer();
      ctx.old.ref = fbutil.ref('homes', ctx.old.uid);
    ...
    function copyProfile() {
      var d = $q.defer();
      ctx.curr.ref = fbutil.ref('homes', ctx.curr.uid);
